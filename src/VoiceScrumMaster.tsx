...
export default function VoiceScrumMaster() {
  ...
  const generateSummaryText = () => {
    ...
    const totalTime = summary.participants.reduce((total, p) => total + p.elapsedSec, 0);
    const avgTimePerPerson = summary.participants.length > 0 
      ? Math.round(totalTime / summary.participants.length) 
      : 0;
    ...
    text += `   • Average Time per Person: ${avgTimePerPerson}s\n`;
    text += `\n`;
